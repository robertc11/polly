...
export default function CreatePostForm(props){
    return (
        <>
            <div className={ !props.view ? "hidden fixed opacity-0 top-0 left-0 right-0 bottom-0 bg-slate-600/50 transition-opacity" : "fixed opacity-1 top-0 left-0 right-0 bottom-0 bg-slate-600/75 flex justify-center items-center transition-opacity" }>
                <div className="m-auto bg-white p-10 w-1/2 relative rounded-sm transition-all ease-in-out">
                    <div className="w-full relative h-10">
                        <button onClick={props.onClose} className="text-4xl absolute right-5">&times;</button>
                    </div>
                    ...
                    <form onSubmit={props.onSubmit} className="flex flex-col">
                        ...
                        <p className={props.errormsg===''?'hidden':'text-violet-400'}>{props.errormsg}</p>
                        
                        <div className="w-full flex items-center justify-center mt-3">
                            <button type="submit" className="w-1/6 mt-3 px-1 py-2 bg-emerald-300 rounded-md mr-1">Create!</button>
                            <button className="w-1/6 mt-3 px-1 py-2 bg-slate-300 rounded-md ml-1" onClick={props.onClose}>Cancel</button>                           
                        </div>
                    </form>
                </div>
            </div>
