...
async function getOneBulletin(client, postID){
    console.log('> dbbulletins.js/getOneBulletin: post to find:',postID)

    try{
        const bulletinDatabase = client.db('bulletin')
        let findResult = await bulletinDatabase.collection('bulletinposts').findOne({ _id: ObjectId(postID) })
...
async function updateBulletin(client, postID, newTitle, newMap, newMapLink, newBody){
    console.log('> dbbulletins.js/updatePost', postID, newTitle, newMap, newMapLink, newBody)
